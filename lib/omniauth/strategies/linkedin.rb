...
module OmniAuth
  module Strategies
    class LinkedIn < OmniAuth::Strategies::OAuth
      ...
      option :fields, ["id", "email-address", "first-name", "last-name", "headline", "summary", "industry", "picture-url", "picture-urls::(original)", "public-profile-url", "location"]
      ...
      info do
        ...
        name = nil_if_empty(name)
        {
          ...
          :image => (raw_info['pictureUrls'] && raw_info['pictureUrls']['values']&.first) || raw_info['pictureUrl'],
          :phone => nil,
